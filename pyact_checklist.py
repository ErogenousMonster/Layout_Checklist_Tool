...
def topology_extract2(start_net_name, start_sch_name):
    ...
    for line in topology_seg_ind:
        ...
        if etch_line_old.GetConnectedSCHListBySegInd(line[-1]):
            ...
            # #print('sch_list_in', sch_list, pin_list)
        ...
        net_nochange_list = copy.deepcopy(net_list)
        next_nochange_pin_list = copy.deepcopy(next_pin_list)
        sch_nochange_list = sch_list
        for ind in xrange(len(sch_nochange_list)):
            ...
            for idxx_ in xrange(len(next_net)):
                ...
                # topology_list = topology_list_final + topology_format(start_net_name, line)
                topology_list = topology_format(start_net_name, line)

                net_nochange_list.append(next_net[idxx_])
                next_nochange_pin_list.append(next_pin[idxx_])
                # sch_list.append(sch_nochange_list[ind])
                # 如果存在信号线，则存入信号线所经过的芯片名称
                if idxx_ > 0:
                    sch_nochange_list.append(sch_nochange_list[-1])
                # 没看懂什么意思
                if net_nochange_list[-1] in non_signal_net_list:
                    topology_list[-1].append(net_nochange_list[-1])

            net_list = copy.deepcopy(net_nochange_list)
            next_pin_list = copy.deepcopy(next_nochange_pin_list)
            sch_list = copy.deepcopy(sch_nochange_list)
                    ...
                    if net_list[ij1] not in non_signal_net_list + [None]:
                        ...
                        try:
                            ...
                            for ind in xrange(len(topology_seg_ind)):
                                ...
                                #     for xxx in range(len(format_item_list) - 1):
                                #         format_flag = True
    ...
    for idxxxx in xrange(len(signal_type_list)):
        ...
        for check_sch_name in start_sch_name_list:
            ...
            for check_net_name in check_net_list:
                ...
                # if check_net_name == 'P3E_CPU0_PE2_TXP0':
                # if check_net_name == 'USB3_DTX_P1':USB1_USB30_TX_N
                # if check_net_name == 'P3E_CPU0_PE2_TXP0':
...
if __name__ == '__main__':
    ...
    for i in sys.argv:
        if i.find('.xlsm') > -1:
            xlsm_path = r'%s' % i
            break
    # xlsm_path = r'C:\Users\KiKi\Desktop\ENT20\ENT20_CML_DT_4Layer_SIM_Checklist_A2.0_20190702.xlsm'
    # # xlsm_path = r'C:\Users\KiKi\Desktop\another\D10_Tahaa CML_B00_SIM_Checklist_A2.2_20190624.xlsm'
    #
    # # xlsm_path = r'C:\Users\KiKi\Desktop\PCIE_Error\FLATIRON_DB_X1_SIM__Checklist_A1.0_20190614.xlsm'
